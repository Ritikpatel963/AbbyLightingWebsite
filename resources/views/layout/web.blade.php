...
<body class="{{ Request::is('/') ? 'home-page' : '' }}">
    ...
    <script type="text/javascript">
        document.addEventListener("DOMContentLoaded", function() {
            document.getElementById("search-icon-mobile").addEventListener("click", function() {
                const display = document.getElementById("search-input-mobile").style.display
                document.getElementById("search-input-mobile").style.display = display == 'block' ? 'none' :
                    'block';
            })
        });
    </script>
    ...
    <script>
        window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-48B9N75V8M');
    </script>
    @stack('js')
</body>
</html>
