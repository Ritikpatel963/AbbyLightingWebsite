            ...
            <div class="p-4">
                <form action="{{ $action }}" method="POST">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4 mb-3">
                        ...
                        <button type="button" class="btn btn-premium btn-info" id="add-value"><i
                                class="ft-plus mr-1"></i> Add Value</button>
                    </div>
                    ...
                    <div id="values-container" class="mb-4">
                        @foreach($attribute->values as $index => $val)
                            ...
                            @endphp
                            <div
                                class="spec-card p-3 mb-3 shadow-sm rounded border d-flex justify-content-between align-items-center value-row">
                                <div class="flex-grow-1 mr-3">
                                    ...
                                    <input type="text" name="values[{{ $index }}][name]" class="form-control" required
                                        value="{{ $val->name }}">
                                </div>
                                <div class="flex-grow-1 mr-3">
                                        ...
                                        <div class="color-input-container">
                                            <div class="mb-1 d-flex align-items-center" style="gap: 5px;">
                                                <select class="form-control color-type-select" style="width: auto;">
                                                    ...
                                                    <option value="gradient" {{ $isGradient ? 'selected' : '' }}>Gradient
                                                    </option>
                                                </select>
                                                <div class="solid-picker"
                                                    style="{{ $isGradient ? 'display: none;' : 'display: block;' }}">
                                                    <input type="color" class="form-control p-1 color-1"
                                                        style="max-width: 50px; cursor: pointer; height: 38px;"
                                                        value="{{ !$isGradient ? $valHex : $gradColor1 }}">
                                                </div>
                                                <div class="gradient-pickers"
                                                    style="{{ $isGradient ? 'display: flex;' : 'display: none;' }} align-items: center; gap: 5px;">
                                                    <input type="color" class="form-control p-1 color-1"
                                                        style="max-width: 50px; cursor: pointer; height: 38px;"
                                                        value="{{ $gradColor1 }}">
                                                    <input type="color" class="form-control p-1 color-2"
                                                        style="max-width: 50px; cursor: pointer; height: 38px;"
                                                        value="{{ $gradColor2 }}">
                                                    <div class="input-group" style="width: 100px;">
                                                        <input type="number" class="form-control gradient-angle"
                                                            value="{{ $gradAngle }}">
                                            ...
                                            <input type="text" name="values[{{ $index }}][hex_code]"
                                                class="form-control final-color-output" placeholder="#ffffff"
                                                value="{{ $valHex }}">
                                        </div>
                                    @else
                                        <input type="text" name="values[{{ $index }}][hex_code]" class="form-control"
                                            placeholder="Leave blank for non-colors" value="{{ $val->hex_code }}">
                                    @endif
                                </div>
                                <div>
                                    <label class="d-block">&nbsp;</label>
                                    <button type="button" class="btn btn-danger remove-val"><i
                    ...
                    <hr class="mt-4 mb-4">
                    <button type="submit" class="btn btn-premium btn-primary"><i class="ft-save mr-1"></i> Update
                        Attribute</button>
                </form>
            </div>
        </div>
